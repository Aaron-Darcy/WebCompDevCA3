...
const CustomerPage = () => {
    ...
    const handleSelectBook = (book) => {
        ...
        setQuantity(1); // Reset quantity when selecting a new book
    };
                    ...
                    <div className="flex-grow space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredBooks.map(book => (
                                ...
                                >
                                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{book.title}</h3>
                                    <p className="text-sm text-gray-600 dark:text-gray-400">{book.author}</p>
                                    <p className="text-sm text-gray-500 dark:text-gray-500">{book.category}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="w-full max-w-md space-y-4">
                        ...
                        {selectedBook && (
                            <div className="mb-4 p-6 border rounded-lg shadow-lg bg-gray-50 dark:bg-gray-800">
                                ...
                                <p className="text-gray-800 dark:text-gray-300">Author: {selectedBook.author}</p>
                                <p className="text-gray-800 dark:text-gray-300">ISBN: {selectedBook.ISBN}</p>
                                <p className="text-gray-800 dark:text-gray-300">Price: ${selectedBook.price}</p>
                                <div className="flex items-center mt-2">
                                    <label className="mr-2 text-gray-800 dark:text-gray-300">Quantity:</label>
                                    <input
                                        type="number"
                                        min="1"
                                        max="5"
                                        value={quantity}
                                        onChange={(e) => setQuantity(Math.max(1, Math.min(5, parseInt(e.target.value))))}
                        ...
                        <div className="p-4 border rounded-lg shadow-lg bg-gray-50 dark:bg-gray-800">
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Your Cart</h3>
                            {cart.length === 0 && <p className="text-gray-800 dark:text-gray-300">Your cart is empty.</p>}
                            {cart.map(item => (
                                <div key={item.book._id} className="flex justify-between items-center mb-2">
                                    <span className="text-gray-800 dark:text-gray-300">{item.book.title} - {item.quantity} copy/copies</span>
                                    <span className="text-gray-800 dark:text-gray-300">${(item.book.price * item.quantity).toFixed(2)}</span>
                                </div>
                            ))}
                        </div>
                    </div>
